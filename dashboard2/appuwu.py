...
# Import model and prediction functions
from terraflow import load_model_and_scalers, get_velocity_data, predict_velocity_at_timestamp
...
def load_models():
    # Velocity Model
    vel_model, vel_scaler_X, vel_scaler_y, device = load_model_and_scalers(
        "models/transformer_model.pth", 
        "models/scaler_X.save", 
        "models/scaler_y.save"
    )
    # Temperature Model
    temp_model, temp_scaler = load_tempflow(
        "models/best_tempflow_model.keras",
        "models/scaler_tempflow.save"
    )
    return vel_model, vel_scaler_X, vel_scaler_y, device, temp_model, temp_scaler

vel_model, vel_scaler_X, vel_scaler_y, device, temp_model, temp_scaler = load_models()
...
with st.sidebar:
    ...
    if location_preset == "Shishper Glacier, Pakistan":
        default_lat, default_lon = 36.47375, 74.70912
    elif location_preset == "Khurdopin Glacier":
        default_lat, default_lon = 36.35835, 75.45967
    elif location_preset == "Passu Glacier":
        default_lat, default_lon = 36.42050, 74.86881
    else:
        default_lat, default_lon = 36.47375, 74.70912
...
# ----------------------------
# Prediction Functions for Tomorrow
# ----------------------------
def get_velocity_prediction():
    """
    Automatically fetches velocity data and generates a forecast for tomorrow.
    """
    try:
        with st.spinner("Fetching velocity data..."):
            common_ts, velocities = get_velocity_data()
        with st.spinner("Generating velocity forecast..."):
            ...
            pred, ts = predict_velocity_at_timestamp(
                vel_model, vel_scaler_X, vel_scaler_y, device,
                common_ts, target_timestamp, (lat, lon)
            )
            return pred, ts
...
# ----------------------------
# 7-Day Forecast Functions - Enhanced
# ----------------------------
def get_velocity_forecast(days=7):
    try:
        common_ts, velocities = get_velocity_data()
        forecast = []
        current_velocities = velocities.copy()  # Start with historical data
        
        for i in range(1, days + 1):
            forecast_date = datetime.datetime.today() + timedelta(days=i)
            target_timestamp = forecast_date.strftime('%Y-%m-%d %H:%M:%S')
            
            # Predict next day's velocity using the latest data
            pred, ts = predict_velocity_at_timestamp(
                vel_model, vel_scaler_X, vel_scaler_y, device,
                common_ts, target_timestamp, (lat, lon)
            )
            
            # Append prediction to input data for next iteration
            new_timestamp = pd.to_datetime(target_timestamp)
            common_ts = np.append(common_ts, new_timestamp)
            current_velocities = np.append(current_velocities, pred)
            
            # Retain only the last 32 timestamps for model input
            if len(common_ts) > 32:
                common_ts = common_ts[-32:]
                current_velocities = current_velocities[-32:]
...
with tabs[0]:
    ...
    with col1:
        ...
        if result:
            ...
            st.metric(
                ...
                delta_color="inverse"  # Higher velocity = bad
            )
            st.caption(f"Data up to {pd.to_datetime(timestamp).strftime('%Y-%m-%d %H:%M')}")
            
            # Add small trend visualization
            last_3months = [85.65, 86.12, 86.78, velocity]
            dates = [
                (datetime.datetime.today() - timedelta(days=90)).strftime('%Y-%m-%d'),
                (datetime.datetime.today() - timedelta(days=60)).strftime('%Y-%m-%d'),
                (datetime.datetime.today() - timedelta(days=30)).strftime('%Y-%m-%d'),
                (datetime.datetime.today() + timedelta(days=1)).strftime('%Y-%m-%d')
            ]
            
            mini_df = pd.DataFrame({
                "Date": dates,
                "Velocity": last_3months
            })
            
            fig = px.line(mini_df, x="Date", y="Velocity", markers=True)
            fig.update_layout(
                height=150,
            ...
            fig.update_traces(line_color="#0078D4")
    ...
    with col2:
        ...
        if temp_pred is not None:
            st.metric(
                ...
                delta_color="inverse"  # Higher temp = bad for glaciers
            )
            tomorrow = datetime.datetime.today() + timedelta(days=1)
            st.caption(f"Forecast for {tomorrow.strftime('%Y-%m-%d')}")
            
            # Add mini temperature trend
    ...
    # Second row: Map and visualizations
    ...
    with map_col:
        ...
        folium_static(m)  # Updated parameter
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with details_col:
        ...
        # Real metrics from available data
        if result:
            velocity, _ = result
        ...
        # Add meaningful calculated metrics
        try:
...
with tabs[1]:
    ...
    if velocity_forecast and temperature_forecast:
        # Prepare data
...
# ----------------------------
    ...
    with col_hist1:
        ...
        if common_ts is not None and velocities is not None:
            # Ensure arrays are same length
            min_length = min(len(common_ts), len(velocities))
            
            # Convert velocities to 1D if necessary
            velocities = np.array(velocities).flatten()

            df_vel = pd.DataFrame({
                "Timestamp": pd.to_datetime(common_ts[:min_length]),
                "Velocity": velocities[:min_length]  # Ensure same length
            }).drop_duplicates().sort_values("Timestamp")
            
            fig = px.line(df_vel, x="Timestamp", y="Velocity", 
                        title="5-Year Velocity Trends",
            ...
            fig.add_trace(
                go.Scatter(
                    x=df_vel["Timestamp"],
                    y=df_vel["Velocity"].rolling(30, min_periods=1).mean(),
            ...
            fig.update_layout(
                ...
                xaxis_range=[df_vel["Timestamp"].min() - timedelta(days=30),
                            df_vel["Timestamp"].max() + timedelta(days=30)]
            )
